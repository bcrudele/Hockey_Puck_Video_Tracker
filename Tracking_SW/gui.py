from PyQt6.QtWidgets import (QApplication, QMainWindow, QPushButton, QVBoxLayout,
                             QWidget, QFileDialog, QLabel, QLineEdit)
...
class VerifyInitWindow(StyledWindow):
    def __init__(self, switch_window):
        ...
        global LH, LS, LV, UH, US, UV
        self.tracker = trackHSV(0, (LH, LS, LV), (UH, US, UV))
        self.tracker.run()
...
class RecordingWindow(StyledWindow):
    def __init__(self, switch_window):
        ...
        self.start_button.setStyleSheet("background-color: #00d002; color: white; padding: 10px; border-radius: 5px;")
        ...
        self.reinit_button.clicked.connect(lambda: self.switch_window("init"))

        self.proceedButton = QPushButton("Procced to Save")
        self.proceedButton.setFont(QFont("Arial", 14))
        self.proceedButton.setStyleSheet("background-color: #1224ec; color: white; padding: 10px; border-radius: 5px;")
        self.proceedButton.setEnabled(False)
        self.proceedButton.clicked.connect(lambda: self.switch_window("save"))
    ...
    def start_tracking(self):
        ...
        global LH, LS, LV, UH, US, UV
        self.tracker = switchtest.trackHSV(0, (LH, LS, LV), (UH, US, UV))
        self.tracker.run(1, 0)

    def stop_tracking(self):
        self.proceedButton.setEnabled(True)
        switchtest.killPPL()
        self.tracker = trackHSV(0, (0, 0, 0), (0, 0, 0))
        self.tracker.run()
        #self.tracker.killHSV()
        return
