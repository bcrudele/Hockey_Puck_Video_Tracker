...
def frame_enqueue(video_path, frame_queue):
    """
...
def process_video(video_path=0, model="yolov5s.pt", frame_skip_en=True, frame_skip=5, gui=True, debug=True, bound=0.1):
    ...
    # set up FIFO queue for frames:
    frame_queue = Queue()
    frame_thread = threading.Thread(target=frame_enqueue, args=(video_path, frame_queue))
    frame_thread.start()
...
if __name__ == '__main__':
    ...
    process_video(video_path, frame_skip_en=True, frame_skip=5, gui=True, debug=False, bound=0.4)
